...
import { Clock, Calendar, FileText, RefreshCw, Mail, Cloud, Loader2 } from 'lucide-react';
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            {action.action_type === 'create_calendar_event' && action.calendar_event_template?.title_template && (
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 truncate">
                {isRTL ? 'אירוע:' : 'Event:'} {action.calendar_event_template.title_template}
              </p>
            )}

            {action.action_type === 'send_email' && action.auto_reply_template && (
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 truncate">
                {isRTL ? 'תבנית:' : 'Template:'} {action.auto_reply_template.substring(0, 40)}...
              </p>
            )}
          </div>
        </div>
