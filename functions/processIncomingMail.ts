...
// פונקציה חיצונית לקבלת קונפיגורציה
...
async function refreshGoogleToken(refreshToken, connectionId, adminBase44) {
    ...
    const encryptedAccess = await encrypt(newAccessToken); // encrypt פונקציה מקומית
    const expiresAt = Date.now() + ((data.expires_in || 3600) * 1000);

    // כאן משתמשים ב-Service Client כי זו כתיבה לטבלת חיבורים
...
// נדרשת פונקציית הצפנה מקומית גם כאן בשביל ה-Refresh
...
Deno.serve(async (req) => {
    ...
    try {
        // קליינט משתמש (User Client) לשליפת נתונים - בטוח ויציב
        const base44 = createClientFromRequest(req);
        const user = await base44.auth.me();
        if (!user) return new Response(JSON.stringify({ error: 'Unauthorized' }), { status: 401, headers });

        // קליינט שרת (Service Client) - רק לגיבוי עבור Refresh Token Write
        const adminBase44 = createClient({ useServiceRole: true });

        console.log("[DEBUG] Finding system connection (User Client)...");
        ...
        if (!connection) {
            return new Response(JSON.stringify({ 
                error: `System connection not found. Please connect Google via Settings.` 
            }), { status: 404, headers });
        }

        console.log("[DEBUG] Connection found. Decrypting...");
        ...
        let savedCount = 0;
        // שימוש בקליינט משתמש רגיל לשמירת מיילים (כי הרשאות Create פתוחות)
