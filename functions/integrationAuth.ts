...
async function handleCallback(code, provider, userId, base44) {
  ...
  if (provider === 'google') {
    const res = await fetch('https://oauth2.googleapis.com/token', {
      ...
      body: new URLSearchParams({ code, client_id: GOOGLE_CLIENT_ID, client_secret: GOOGLE_CLIENT_SECRET, redirect_uri: REDIRECT_URI, grant_type: 'authorization_code' }).toString(),
    });
    ...
    const res = await fetch('https://api.dropboxapi.com/oauth2/token', {
      ...
      body: new URLSearchParams({ code, grant_type: 'authorization_code', redirect_uri: REDIRECT_URI }).toString(),
    });
    tokens = await res.json();
  }

  if (!tokens || tokens.error) throw new Error(`Provider Error: ${JSON.stringify(tokens)}`);

  const encryptedAccess = await encrypt(tokens.access_token);
  const encryptedRefresh = tokens.refresh_token ? await encrypt(tokens.refresh_token) : null;
  const expiresAt = Date.now() + ((tokens.expires_in || 3600) * 1000); 

  // === תיקון קריטי: שימוש ב-asServiceRole כדי להבטיח גישה למסד הנתונים ===
  const existing = await base44.asServiceRole.entities.IntegrationConnection.filter({ user_id: userId, provider });
  
  const data = { user_id: userId, provider, access_token: encryptedAccess, expires_at: expiresAt, metadata: { last_updated: new Date().toISOString() } };
  ...
  if (existing.length > 0) {
    ...
    await base44.asServiceRole.entities.IntegrationConnection.create({ ...data, refresh_token: encryptedRefresh || "MISSING" });
  }
}
